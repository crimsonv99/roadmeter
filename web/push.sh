...
# git config, the keychain, or command history in plaintext output.
set -euo pipefail
...
if [ -z "$TOKEN" ] || [ "$TOKEN" = "PASTE_YOUR_FRESH_GITHUB_TOKEN_HERE" ]; then
  echo "✗ web/.push-token still has the placeholder — paste a real token first." >&2
  exit 1
fi
...
if ! git -C "$REPO_DIR" check-ignore -q "$TOKEN_FILE"; then
  echo "✗ Refusing to run: $TOKEN_FILE is NOT git-ignored. Fix .gitignore first." >&2
  exit 1
fi
...
git -C "$REPO_DIR" \
  -c credential.helper='!f() { echo "username='"$GH_USER"'"; echo "password='"$TOKEN"'"; }; f' \
  push -u origin "$BRANCH" 2>&1 | sed "s/$TOKEN/***TOKEN***/g"